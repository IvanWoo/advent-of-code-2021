...
# https://github.com/anthonywritescode/aoc2021/blob/main/day23/part2.py
POS = {"A": 0, "B": 1, "C": 2, "D": 3}
...
class State(NamedTuple):
    ...
    def completed(self) -> bool:
        return (
            all(k == v for k, v in self.row1.items())
            and all(k == v for k, v in self.row2.items())
            and all(k == v for k, v in self.row3.items())
            and all(k == v for k, v in self.row4.items())
        )
...
def next_states(
    ...
    for k, v in state.top.items():
        ...
        if all(
            k2 <= min_c or k2 >= max_c or v2 is None for k2, v2 in state.top.items()
        ):
    ...
    for i in range(4):
        ...
        for target in potential_targets:
            ...
            if all(
                ...
                if row1_val is not None and (
                    row1_val != i or row2_val != i or row3_val != i or row4_val != i
                ):
                    yield (
                    ...
                    row1_val is None
                    and row2_val is not None
                    and (row2_val != i or row3_val != i or row4_val != i)
                ):
                    yield (
                    ...
                    row1_val is None
                    and row2_val is None
                    and row3_val is not None
                    and (row3_val != i or row4_val != i)
                ):
                    yield (
                    ...
                    row1_val is None
                    and row2_val is None
                    and row3_val is None
                    and row4_val is not None
                    and row4_val != i
                ):
                    yield (
